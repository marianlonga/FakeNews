...
if DO_STATISTICS:
    ...
    for feature in statistics_features:
        ...
        statistics.append([feature, diff_mean_value, diff_mean_percentage, p_value, float(t_value)])

    # select which features to keep
    statistics = [statistic for statistic in statistics if abs(statistic[2]) >= DIFF_MEAN_PERCENTAGE_CUTOFF] # only keep statistics with big enough diff_mean_percentage value
    statistics = [statistic for statistic in statistics if statistic[3] <= P_VALUE_CUTOFF] # only keep statistics with low p_value
    if not INCLUDE_TEXT_FEATURES:
        text_features_prefixes = ['user_name', 'user_screen_name', 'user_description', 'text']
        statistics = [statistic for statistic in statistics if not statistic[0].startswith(tuple(text_features_prefixes))]

    # group features and select the best feature from each group
    statistics = sorted(statistics, key=lambda s: abs(s[2]), reverse=True)  # sort statistics by diff_mean_percentage
    statistics_with_best_feature_per_group = []
    for feature_group in feature_groups:
        statistics_group = [statistic for statistic in statistics if (statistic[0] in feature_group)]
        if statistics_group:
            statistics_with_best_feature_per_group.append(statistics_group[0])
    statistics = statistics_with_best_feature_per_group
    statistics = sorted(statistics, key=lambda s: abs(s[2]), reverse=True)  # sort statistics by diff_mean_percentage
